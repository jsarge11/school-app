...
export default class StudentAssessments extends React.Component {
    ...
    componentDidMount() {
        axios.get('https://sargentassociates.com:4000/math/assessments/student?id=' + this.props.student.st_id).then(res => {
    ...
    handleClick = (id) => {
        ...
        if (assessments.includes(+assessmentValue)) {
            ...
            axios.post('https://sargentassociates.com:4000/math/assessments?id=' + id, {assessmentValue: assessmentValue}).then(res => {
                this.setState({ assessments: res.data[0].assessments })
            })
        }
